...
async function startApp(app:any, port: number, logger: Logger): Promise<void>{
  try {
      ...
      logger.log(`Producer Service is running on: ${port}`);
    } catch (error) {
      logger.error('Error starting Producer Service', error.stack);
      process.exit(1);
  }
}
...
function configureGlobalSettings(app: any): void {
  ...
  app.setGlobalPrefix('ms-producer')    // Prefijo global para las rutas

}

function setupSwagger(app: any): void {
  const config = new DocumentBuilder()
    .setTitle('Producers Service API')
    .setDescription('API documentation for the Producers Service')
    .setVersion('1.0')
    .build();
...
async function bootstrap() {
  ...
  const port          = configService.get<number>('PRODUCER_SERVICE_PORT');

  if (!port){
    logger.error('PRODUCER_SERVICE_PORT is not defined in the environment');
    process.exit(1);
  }
